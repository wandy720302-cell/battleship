...
- **上班模式（Excel，可玩）**：按 `Esc`（或頂欄 📊）變成一張 Q3 業績試算表，分頁標題與 favicon 一起換、自動靜音。左表 Forecast = 敵方海域、右表 Actual = 我方（淡藍填色是船）。**方向鍵移動、Enter 開火**，滑鼠點兩下同一格也行。命中 = 紅字負數、落空 = `-`、擊沉 = 淺紅填滿深紅字。狀態列左邊 `Ready` = 輪到你、`Calculating…` = 等對手；右邊 `Sum` = 對方沉幾艘、`Count` = 我剩幾艘。擺船階段 `F9`（Excel 重算鍵）= 隨機部署、Enter = 準備完成。終局跳 Excel 風格對話框，OK = 再來一局
